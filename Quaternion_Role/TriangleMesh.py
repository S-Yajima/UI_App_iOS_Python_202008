...
class MyFigure():
	...
	# 回転、移動を反映した座標を返す
	# 配列：[[[x,y,z],[x,y,z],[x,y,z]],[[]...
	def local_points_q(self, axis_n_x=0, axis_n_y=0, axis_n_z=0):
		result = []
		
		# 行列計算の関数で移動した座標を算出する
		# 円図形の頂点を算出する
		trans_matrix = make_Translation_Matrix(
			self.center.x, self.center.y, self.center.z)
		
		# 行列計算の関数でクオータニオンにて回転した座標を算出する
		quat_v4 = quaternion_v4(
			self.quat_rotate_angle, axis_n_x, axis_n_y, axis_n_z)
		rotate_quat_matrix = make_Quaternion_Matrix(quat_v4)
		
		
		# 行列同士を掛け算して結合する
		matrix_xyzt = multipl_Matrix(
			trans_matrix, rotate_quat_matrix)
	...
	def local_points(self):
		...
		rotate_matrix_x = make_Rotation_X_Matrix(self.rotate_angle_x)
		
		# 行列計算の関数でY軸に回転した座標を算出する
		rotate_matrix_y = make_Rotation_Y_Matrix(self.rotate_angle_y)
		
		# 行列計算の関数でZ軸に回転した座標を算出する
		rotate_matrix_z = make_Rotation_Z_Matrix(self.rotate_angle_z)
		
		# 行列同士を掛け算して結合する
		rotate_matrix_xy = multipl_Matrix(
			rotate_matrix_y, rotate_matrix_x)
		rotate_matrix_xyz = multipl_Matrix(
			rotate_matrix_z, rotate_matrix_xy)
		matrix_xyzt = multipl_Matrix(
			trans_matrix, rotate_matrix_xyz)
