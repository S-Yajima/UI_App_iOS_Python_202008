...
def quaternion_v4(theta, x, y, z):
	...
	cos_val = cos(radians(theta * 0.5))
	sin_val = sin(radians(theta * 0.5))
...
def make_Scale_With_Vector_Matrix(normal, k):
	...
	matrix[0][0] = 1 + (k - 1) * (n_x**2)
	matrix[0][1] = (k - 1) * n_x * n_y
	matrix[0][2] = (k - 1) * n_x * n_z
	matrix[0][3] = 0
	matrix[1][0] = (k - 1) * n_x * n_y
	matrix[1][1] = 1 + (k - 1) * n_y**2
	matrix[1][2] = (k - 1) * n_y * n_z
	matrix[1][3] = 0
	matrix[2][0] = (k - 1) * n_x * n_z
	matrix[2][1] = (k - 1) * n_y * n_z
	matrix[2][2] = 1 + (k - 1) * n_z**2
	matrix[2][3] = 0
