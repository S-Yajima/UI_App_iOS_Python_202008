		...
		for figure in sorted_figures:
			...
			for triangle in local_points:
				...
				for vertex in triangle:
					...
					g_x, g_y, g_z = self.camera.camera_rotation(g_x, g_y, g_z)
...
if __name__ == '__main__':
	...
	main_view.name = '床に影を行列で描く'
	main_view.background_color = 'lightblue'
	
	# カメラの位置
	main_view.camera.set_camera_position(0, 0, 350)
	# カメラの注視点
	...
	# 数字を回す
	ts1 = threading.Timer(delay, rotate_Y_figure_order, args=[numbers, 0, lock])
	ts1.start()
	
	# 視点移動　上昇
	delay += 0.5
	tv4 = threading.Timer(delay, change_view_y_schedule, args=[main_view, -4*80, -4, lock])
	tv4.start()
	...
	# 光源を動かす
	delay += 1.0
	tml_4 = threading.Timer(delay, move_light_schedule, args=[main_view, [0, -10, 0], 10, lock])
	tml_4.start()
	
	# 視点移動　左右円周
	delay += 2.0
	#tv1 = threading.Timer(delay, change_view_round_schedule, args=[main_view, 110, -1, 20, lock])
	tv1 = threading.Timer(delay, change_view_round_schedule, args=[main_view, 110, -1, 40, lock])
	tv1.start()
	...
	# 光源を動かす
	delay += 3.0
	tml_5 = threading.Timer(delay, move_light_schedule, args=[main_view, [0, 0, -10], 20, lock])
	tml_5.start()
	
	# 光源を動かす
	delay += 3.0
	tml_6 = threading.Timer(delay, move_light_schedule, args=[main_view, [10, 0, 0], 20, lock])
	tml_6.start()
	
	# 光源を動かす
	delay += 3.0
	tml_7 = threading.Timer(delay, move_light_schedule, args=[main_view, [0, 10, 0], 10, lock])
	tml_7.start()
	
	# 光源を動かす
	delay += 3.0
	tml_8 = threading.Timer(delay, move_light_schedule, args=[main_view, [0, 0, -10], 15, lock])
	tml_8.start()
	
	# 光源を動かす
	delay += 3.0
	tml_9 = threading.Timer(delay, move_light_schedule, args=[main_view, [0, 0, 10], 15, lock])
	tml_9.start()
	
	# 光源を動かす
	delay += 3.0
	tml_9 = threading.Timer(delay, move_light_schedule, args=[main_view, [0, 10, 0], 6, lock])
	tml_9.start()
